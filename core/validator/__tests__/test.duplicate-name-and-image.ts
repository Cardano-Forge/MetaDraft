...
Deno.test("DuplicateNameAndImage - withError", () => {
  ...
  assertEquals(result["asset_0000"].errors[1].validationErrors.length, 1);
  assertEquals(
    result["asset_0000"].errors[1].validationErrors[0].message,
    "Image: adibou.png has been detected as a duplicate."
  );
  assertEquals(result["asset_0000"].errors[1].validationErrors[0].path, [
    "image",
  ]);
  ...
  assertEquals(result["asset_0002"].errors[1].validationErrors.length, 1);
  assertEquals(
    result["asset_0002"].errors[1].validationErrors[0].message,
    "Image: adibou.png has been detected as a duplicate."
  );
  assertEquals(result["asset_0002"].errors[1].validationErrors[0].path, [
    "image",
  ]);
});
